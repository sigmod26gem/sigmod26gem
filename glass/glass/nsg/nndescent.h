...
struct NNDescent {
  ...
  void Refine(const float *data, const Graph<int> &src_graph) {
    this->data = data;
    nb = src_graph.N;
    K = src_graph.K;
    L = K + 50;
...
#pragma omp for
  ...
  void InitFrom(const Graph<int> &src_graph) {
    graph.reserve(nb);
    {
      std::mt19937 rng(random_seed * 6007);
      for (int i = 0; i < nb; ++i) {
        graph.emplace_back(rng, src_graph.K, nb);
      }
    }
#pragma omp parallel
    {
#pragma omp for
      for (int i = 0; i < nb; ++i) {
        for (int j = 0; j < src_graph.K; j++) {
          int id = src_graph.at(i, j);
          float dist = dist_func(data + i * d, data + id * d, d);
          graph[i].pool.push_back(Neighbor(id, dist, true));
        }
        std::make_heap(graph[i].pool.begin(), graph[i].pool.end());
