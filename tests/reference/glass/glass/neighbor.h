...
template <typename dist_t> struct LinearPool {
  ...
  void unique_insert(int u, dist_t dist) {
    if (size_ == capacity_ && dist >= data_[size_ - 1].distance) {
      return;
    }
    int lo = find_bsearch(dist);
    if (get_id(data_[lo].id) != get_id(u)) {
      std::memmove(&data_[lo + 1], &data_[lo],
                   (size_ - lo) * sizeof(Neighbor<dist_t>));
      data_[lo] = {u, dist};
      if (size_ < capacity_) {
    ...
    else if (!is_checked(u)) {
      set_unchecked(data_[lo].id);
    }
  }
  ...
  dist_t get_dist_bound() {
    return data_[size_ - 1].distance;
  }
  ...
  void set_unchecked(int &id) { id &= kMask; }
...
template <typename dist_t> struct LinearPoolWithoutBitset {
  LinearPoolWithoutBitset(int n, int capacity, int = 0)
      : nb(n), capacity_(capacity), data_(capacity_ + 1) {}
