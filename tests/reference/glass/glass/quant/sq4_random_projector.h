...
struct SQ4RPQuantizer {
  ...
  SQ4RPQuantizer() = default;

  explicit SQ4RPQuantizer(int dim)
      : d(dim), d_align(do_align(dim, kAlign)), code_size(d_reduced / 2),
        reorderer(dim) {}

  ~SQ4RPQuantizer() { free(codes); free(rndAs); }

  void train(const float *data, int64_t n) {
    rndAs = (float *)alloc2M(d * d_reduced * sizeof(float));
...
#pragma omp parallel for schedule(dynamic)
  ...
  template <int DALIGN = do_align(DIM, kAlign)> struct Computer {
    ...
    const SQ4RPQuantizer &quant;
    uint8_t *q = nullptr;
    Computer(const SQ4RPQuantizer &quant, const float *query)
        : quant(quant), q((uint8_t *)alloc64B(d_reduced / 2)) {
